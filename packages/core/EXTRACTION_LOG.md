...
## Пост-извлечение: чистка документации

- Разовые отчёты удалены из обоих репо (`QUOTE_RUNS_REPORT`,
  `RUN_LENGTH_REPORT`, `online-readiness`, `monkeytype-review`), дубликат
  `AUDIT_STATE.md` в бэке удалён (единственная копия — во фронте),
  разъехавшиеся front-зеркала `MATCH.md`/`PROTOCOL.md` удалены (канон — в
  бэке, ссылки в коде переписаны явно).
- `SCORING_CONCEPT.md` переехал в этот пакет: спецификация скоринга живёт
  рядом с `score.ts`/`mods.ts`, которые реализуют её. Межрепо-дубликат
  удалён; docs-map бэка указывает сюда. Все упоминания в коде — bare-имя
  («SCORING_CONCEPT §2»), путевых ссылок не было, ничего не сломано.
