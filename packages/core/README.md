...
## Спецификация скоринга

[`SCORING_CONCEPT.md`](./SCORING_CONCEPT.md) живёт в этом пакете — рядом с
`score.ts`/`mods.ts`, которые и есть этот документ в виде кода. Раньше он
дублировался в обоих репозиториях; теперь копия одна, бэкенд ссылается сюда.
