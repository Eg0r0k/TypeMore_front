<template>
  <div class="server-modal">
    ...
    <div class="server-modal__search">
      <SearchBar v-model="searchQuery" placeholder="Search room..." aria-label="Search room by name" />
    </div>

    <ul class="server-modal__rooms" aria-live="polite">
      <li v-for="room in filteredRooms" :key="room.id" class="room" role="button" tabindex="0" @click="selectRoom(room)"
        @keydown.enter="selectRoom(room)" @keydown.space="selectRoom(room)" :aria-label="`Select room ${room.name}`">
        <div class="name">
...
.server-modal {
  ...
  &__rooms {
    ...
    .room {
      ...
      user-select: none;
      cursor: pointer;
      grid-template-areas: 'name name name chevron'
