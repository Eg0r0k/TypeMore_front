<template>
  <div class="time-modal">
    <Typography class="time-modal__title" size="l" tag-name="h2" color="primary">Custrom time amount</Typography>
    <TextInput @keydown.enter="saveTimeAmount" placeholder="Set time (e.g., 24h, 20m10s, 1h30m, 3600)"
      v-model="inputTime" v-select min="0">
      <Typography tag-name="span" color="primary" size="xxs">
        {{ formattedTime }}
      </Typography>
    </TextInput>
    <Typography color="sub" size="xs">You can use "h" for hours and "m" for minutes, for example "1h30m".</Typography>
    <Typography size="xs" color="sub">
      You can start an infinite test by inputting 0. Then, to stop the test, use the Bail Out feature
    </Typography>
    <Typography color="sub" size="xs"> ( <kbd>esc</kbd> or <kbd>ctrl/cmd</kbd> + <kbd>shift</kbd> + <kbd>p</kbd> > Bail
      Out )
    </Typography>
...
const formattedTime = computed(() => {
  ...
  if (seconds > 0) {
    formatted += `${hours > 0 || minutes > 0 ? ' and ' : ''
