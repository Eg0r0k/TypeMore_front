  ...
  <div class="recorder">
    ...
    <div class="replay">
      ...
      <p>
        Воспроизведение:
        <span
          v-for="(char, index) in replayedTextArray"
          :key="index"
          :style="{ color: getColorBySpeed(char.interval) }"
        >
          {{ char.key }}
        </span>
      </p>
...
const getColorBySpeed = (interval: number) => {
  ...
  const normalizedInterval = Math.min(
    Math.max((interval - minInterval) / (maxInterval - minInterval), 0),
    1
  )
